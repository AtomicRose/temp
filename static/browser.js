...
(function () {
    ...
    var version = parseInt(browser.version.split('.')[0], 10);
    var os = browser.os;
    var osVersion = parseInt(browser.osVersion.split('.')[0], 10);
    if (device === 'PC') {
        if (browserName === 'IE' && version < 11) {
            browserError()
        }
        if (browserName === 'Chrome' && version < 49) {
            browserError()
        }
        if (browserName === 'Firefox' && version < 59) {
            browserError()
        }
    }
    if (device === 'Mobile') {
    ...
    function browserError() {
        ...
        if (!old.match('/browser')) {
            if (old.match('/en_US/')) {
                window.location.href = '//' + window.location.host + '/en_US/browser'
            } else {
                window.location.href = '//' + window.location.host + '/zh_CN/browser'
            }
        }
    }
